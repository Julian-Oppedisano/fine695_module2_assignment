...
with open('outputs/lasso_params.json', 'w') as f:
    json.dump(result, f)
print('Best params and score saved to outputs/lasso_params.json')
...
# Save fitted model
joblib.dump(pipeline, 'outputs/lasso_model.pkl')
print('Fitted model saved to outputs/lasso_model.pkl')

# Test: reload model
loaded_model = joblib.load('outputs/lasso_model.pkl')
print('Model reloaded successfully')

# Task 14: Generate predictions for stock_exret on test window
test = test.copy()  # Create a copy to avoid fragmentation
test.loc[:, 'lasso_pred'] = loaded_model.predict(test[predictors])
print('Predictions saved to test DataFrame as lasso_pred')
print('All predictions non-null:', test['lasso_pred'].notna().all())

# Task 15: Compute out-of-sample R² score
oos_r2 = r2_score(test['stock_exret'], test['lasso_pred'])
print('Out-of-sample R² score:', oos_r2)

# Append to metrics.csv
metrics = pd.DataFrame({'model': ['lasso'], 'oos_r2': [oos_r2]})
...
lasso_port_ret = form_portfolio(test, 'lasso_pred', n_stocks=50)
lasso_port_ret.columns = ['lasso_port_ret']  # Rename to match model name
lasso_port_ret.to_csv('outputs/lasso_port_ret.csv')
print('Monthly portfolio returns saved to outputs/lasso_port_ret.csv')
...
with open('outputs/lasso_params.json') as f:
    print(json.load(f)) 