...
with open('outputs/elasticnet_params.json', 'w') as f:
    json.dump(result, f)
print('Best params and score saved to outputs/elasticnet_params.json')
...
# Save fitted model
joblib.dump(pipeline, 'outputs/elasticnet_model.pkl')
print('Fitted model saved to outputs/elasticnet_model.pkl')

# Test: reload model
loaded_model = joblib.load('outputs/elasticnet_model.pkl')
print('Model reloaded successfully')

# Task 14: Generate predictions for stock_exret on test window
test = test.copy()  # Create a copy to avoid fragmentation
test.loc[:, 'elasticnet_pred'] = loaded_model.predict(test[predictors])
print('Predictions saved to test DataFrame as elasticnet_pred')
print('All predictions non-null:', test['elasticnet_pred'].notna().all())

# Task 15: Compute out-of-sample R² score
oos_r2 = r2_score(test['stock_exret'], test['elasticnet_pred'])
print('Out-of-sample R² score:', oos_r2)

# Append to metrics.csv
metrics = pd.DataFrame({'model': ['elasticnet'], 'oos_r2': [oos_r2]})
...
elasticnet_port_ret = form_portfolio(test, 'elasticnet_pred', n_stocks=50)
elasticnet_port_ret.columns = ['elasticnet_port_ret']  # Rename to match model name
elasticnet_port_ret.to_csv('outputs/elasticnet_port_ret.csv')
print('Monthly portfolio returns saved to outputs/elasticnet_port_ret.csv')
